...
class ChatGPT:
    def __init__(self, model="gpt-4o"):
        assert model in {"text-davinci-003", "gpt-3.5-turbo-16k", "gpt-3.5-turbo", "gpt-4", "gpt-4o", "gpt-4o-2024-05-13", "gpt-4-1106-preview", "gpt-4-vision-preview"}, f"Unknown model: {model}"
        self.model = model
    ...
    async def send_message(self, message, dialog_messages=[], chat_mode="assistant"):
        ...
        while answer is None:
            try:
                if self.model in {"gpt-3.5-turbo-16k", "gpt-3.5-turbo", "gpt-4", "gpt-4o", "gpt-4o-2024-05-13", "gpt-4-1106-preview", "gpt-4-vision-preview"}:
    ...
    async def send_message_stream(self, message, dialog_messages=[], chat_mode="assistant"):
        ...
        while answer is None:
            try:
                if self.model in {"gpt-3.5-turbo-16k", "gpt-3.5-turbo", "gpt-4","gpt-4o", "gpt-4o-2024-05-13", "gpt-4-1106-preview"}:
                    messages = self._generate_prompt_messages(message, dialog_messages, chat_mode)
                    ...
                    r_gen = await client.responses.create(
                        model=self.model,
                        prompt=prompt,
                        input=messages,
                        stream=True,
                    ...
                    async for r_item in r_gen:
                        if r_item.type == "response.output_text.delta":
                            delta = r_item.delta or ""
                            answer += delta
    ...
    async def send_vision_message(
        ...
        while answer is None:
            try:
                if self.model == "gpt-4-vision-preview" or self.model == "gpt-4o" or self.model == "gpt-4o-2024-05-13":
    ...
    def summarize_messages(messages: list):
        input_items = [
            {
                "role": "system",
    ...
    def _count_tokens_from_messages(self, messages, answer, model="gpt-4o"):
        ...
        if model == "gpt-3.5-turbo-16k":
            ...
            tokens_per_message = 3
            tokens_per_name = 1
        elif model == "gpt-4o" or model == "gpt-4o-2024-05-13":
            tokens_per_message = 3
            tokens_per_name = 1
        else:
...
async def generate_images(prompt, n_images=4, size="512x512"):
    r = await openai.images.generate(prompt=prompt, n=n_images, size=size)
    image_urls = [item.url for item in r.data]
    return image_urls


async def is_content_acceptable(prompt):
    r = await openai.moderations.create(input=prompt)
